...
#目录
train_query_dir = './samples/question'#'./samples/train.query'
train_answer_dir = './samples/answer'#'./samples/train.answer'

# 放在全局的位置，为了动态算出num_encoder_symbols和num_decoder_symbols
max_token_id = wordToken.load_file_list([train_query_dir,train_answer_dir], min_freq)
...
def get_train_set():
    ...
    train_set = []
    ##################分割线
    with open(train_query_dir, 'r', encoding='utf-8') as question_file:
        with open(train_answer_dir, 'r', encoding='utf-8') as answer_file:
            while True:
